...
class SettingTabs extends Tabs
{
    ...
    private function moniq()
    {
        return tap(new Tab('moniq', trans('setting::settings.tabs.moniq')), function (Tab $tab) {
            $tab->weight(72);

            $tab->fields([
                'moniq_enabled',
                'translatable.moniq_label',
                'translatable.moniq_description',
                'moniq_public_key',
                'moniq_api_secret',
            ]);

            $tab->view('setting::admin.settings.tabs.moniq');
        });
    }
