...
class CheckoutCompleteController
{
    ...
    public function store(int $orderId, OrderService $orderService)
    {
        ...
        if (request()->query('paymentMethod') === 'moniq') {
            $redirectToCancel = redirect()->route('checkout.payment_canceled.store', [
                'orderId' => $orderId,
                'paymentMethod' => 'moniq'
            ]);
            ...
            try {
                ...
                $moniq = new MoniqService($moniqConfig);

                // Get stored API order ID from order notes
                $notes = json_decode($order->note ?? '{}', true);
                $apiOrderId = $notes['moniq_order_id'] ?? null;

                if ($apiOrderId) {
                    $paymentResponse = $moniq->verifyPayment($apiOrderId);
                ...
                Log::error('Moniq callback error: ' . $e->getMessage());
